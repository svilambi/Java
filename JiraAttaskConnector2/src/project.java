...
public class project {
   ...
   // Folder inbox = store.getFolder("Inbox/Test"); 
    ...
    cal.add(Calendar.DAY_OF_MONTH, -24);                //add 1 day
    ...
    //  Message[] messages = inbox.getMessages();
     ...
     //String regex2 = "\\[I#(.*?)\\]";
      ...
      for (int i = n-1;  i >= 0; --i) {
        ...
        // String mailLines[] = mailContent.split("\\s*[Changeset: ]+\\s*");
         ...
         for (int j = 0; j < mailLines.length; j++) {
        	 ...
        	 while (index > 0) {
        	     ...
        	     changeSet = mailLines[j].substring(index+10, index+16 );
        	     ...
        	     while(matcher5.find()&&matcher2.find()){
        	    	 ...
        	    	 System.out.println("user name is"+usr);
            	       ...
            	       /*	JSONObject issueDatalnk=null;
            	        	...
            	        	try{
            	        		String issuelnk = invokeGetMethod(auth, BASE_URL+"/rest/api/2/issue/"+str+"?fields=issuelinks");
            	        	...
            	        	try{
            					...
            					String editIssueData = "{\"body\": \"patched in "+ message.getSubject()+" and Changeset number is: "+changeSet+"\"}";
            	        	...
            	        	/*catch (Exception msg) {
            	    			...
            	    			try{
            	    			   ...
            	    			   // msg.addRecipients(Message.RecipientType.CC, InternetAddress.parse("BuildTeam@amphorainc.com"));
            	    			    ...
            	    			    messageBody.setText("You might have given wrong Issue Number(or format of Issue number) as "+str+" for changeset "+changeSet+" which is not found in JIRA. Please correct.");
